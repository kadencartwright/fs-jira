...
export function WorkspacesCard({
  ...
  return (
    <section className="rounded-xl border border-slate-700/80 bg-slate-900/70 p-4 shadow-sm shadow-black/20">
      ...
      {loadError ? (
        <p className="mb-2 rounded-md border border-red-500/40 bg-red-950/40 p-2 text-xs text-red-200">
          {loadError}
        </p>
      ) : null}
      ...
      <div className="space-y-2">
        {rows.map((row, index) => {
          ...
          return (
            <div
              className="rounded-md border border-slate-700/70 bg-slate-950/60 p-3"
              key={`${index}-${row.name}`}
            >
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-5">
                <input
                  className="rounded-md border border-slate-600 bg-slate-900 px-2 py-2 text-sm text-slate-100 sm:col-span-1"
                  onChange={(event) => {
                ...
                />
                <input
                  className="rounded-md border border-slate-600 bg-slate-900 px-2 py-2 text-sm text-slate-100 sm:col-span-4"
                  onChange={(event) => {
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <button
          className="rounded-md border border-slate-600 px-3 py-2 text-sm text-slate-100 hover:bg-slate-800"
          disabled={busy}
          onClick={() => {
        ...
        <button
          className="rounded-md bg-blue-500 px-3 py-2 text-sm font-medium text-white hover:bg-blue-400 disabled:cursor-not-allowed disabled:bg-slate-700"
